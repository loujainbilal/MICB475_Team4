...
abundance_daa_results_df <- pathway_daa(abundance = abundance_data_filtered %>% column_to_rownames("pathway"), 
                                        metadata = meta, group = "fast_glucose_prediabetic", daa_method = "DESeq2")
...
abundance_desc = abundance_desc[,-c(441:ncol(abundance_desc))] 

# Generate a heatmap
pathway_heatmap(abundance = abundance_desc %>% column_to_rownames("feature"), metadata = meta, group = "fast_glucose_prediabetic")

# Generate pathway PCA plot
pathway_pca(abundance = abundance_data_filtered %>% column_to_rownames("pathway"), metadata = meta, group = "fast_glucose_prediabetic")
...
res =  DEseq2_function(abundance_data_filtered, meta, "fast_glucose_prediabetic")
